<?php
$Foxgo=new Foxgo();
$Foxgo->is_https=false;  //"true" is use https ,"false" is use http
$Foxgo->domain="www.shopstyle.com";
...
$Foxgo->fullURL=$Foxgo->translateServer().str_replace($Foxgo->php_self(),"",$Foxgo->path);
$Foxgo->connect();
$Foxgo->output();
class Foxgo{
...
//		}
	...
	//	foreach($result as $i=>$value){
		...
		$html = preg_replace("#(src|href)=(\"|')(/|(?!http))(.*?)(\"|')#", "$1=\"http://".$this->host.$this->scriptname."/$4\"", $html);
	...
	function googleurl(){
		...
		return "http://cpi.".$google."/sd_check.php";	
	}
}
